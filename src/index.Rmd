---
title: "What makes a Resource-Constrained Project Scheduling Problem (RCPSP) instance difficult?"
output:
...
The Resource Constrained Project Scheduling Problem (RCPSP) which is a well-known and extensively studied operations research (OR) problem.
In general, an RCPSP is a project (eg. building a house), characterized by several activities (eg. digging, building the floor, building the walls, adding electricity, adding windows,...) which all require a certain number of limited resources to be executed (eg. there are only 5 manual labourers available at the same time).
...
The goal is to schedule the activities to minimize the total time to complete the project while respecting all precedence constraints and the resource constraints.
...
The figure below shows a toy example of an RCPSP.
...
The bottom figure shows the optimal solution where each activity is scheduled, respecting all precedence constraints .

![A toy RCPSP example](rcpsp_example.png)

The aim of this paper is to investigate what makes an RCPSP hard to solve for the currently existing algorithms. The next two sections explain (1) the dataset that we will work with and (2) how one can describe the complexity of an RCPSP instance.

## Data source and description

Over the years, several instance sets have been developed for researchers to test new algorithms.
In many papers, researchers will report how many instances of the set they can solve and how long it takes to solve them to show the quality of their new algorithm.
...
Any researcher can contribute their results and these results are openly available to download and compare.

In this article, we first explore the available datasets for the following datasets:

* PSPLIB dataset: although these datasets have been available for a long time, some instances have still not been solved [@kolisch1997psplib]. They are very well known and commonly referred to as "J##" where ## denote the number of activities in the network. Popular problem instances are J30, J60, J90 and J120.
...
Over the years, the list of possible parameters has grown a lot and each may have its advantages and disadvantages.

For each instance on the [website](http://solutionsupdate.ugent.be/rcpsp), several parameters describe the network topology, complexity, and resources (Table \@ref(tab:parameter)).  
...
The correlation matrices for the CV instances (Figure \@ref(fig:cv-cormat) and \@ref(fig:cv-cormat-reorder)) show that some correlations are problematically high since the parameters measure almost the same thing.
...
```
```{r cv-tree-all-km, fig.cap="Kaplan-Meier curves for the different groups for the survival tree for the whole dataset.", echo = FALSE, message = FALSE, warning = FALSE}
KMPlot(out.tree, scale.time=1000, type=1)
```

```{r cv-tree-C, fig.cap="A survival tree for the Creemers algorithm.", echo = FALSE, message = FALSE, warning = FALSE}
par(xpd = TRUE)
plot(out.tree.c, compress = TRUE)
text(out.tree.c, use.n = TRUE)
```
```{r cv-tree-C-km, fig.cap="Kaplan-Meier curves for the different groups for the survival tree for the Creemers algorithm.", echo = FALSE, message = FALSE, warning = FALSE}
