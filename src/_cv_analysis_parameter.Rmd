...
data_cv_all_2 <- data_cv %>%
  mutate(binned = chop_equally(x = data_cv %>% dplyr::pull(this_par), groups = 4)) %>%
  dplyr::select(IDset, binned) %>%
  right_join(data_cv_all_1, by = join_by(IDset == ID))
...
The `r this_par` parameter takes values between `r min(data_cv %>% dplyr::pull(this_par))` and `r max(data_cv %>% dplyr::pull(this_par))`. `r this_par` is defined as "`r parameters %>% dplyr::filter(Parameter == this_par) %>% dplyr::pull(Info)`". It is split up in `r nb_groups` equal-sized discrete bins (same number of instances in each bin). We now compare the run times for instances in each of the `r nb_groups` bins for both @watermeyer2022partition and @creemers2021.
...
p1 <- survfit2(Surv(Time, status) ~ binned,
  ...
  scale_color_manual(name = this_par,
                     values = colorRampPalette(colors = c("#99CCFF", "#003366"))(nb_groups))
